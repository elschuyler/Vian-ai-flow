...
let projects         = [];
const sessionStore   = new Map(); // [STORE]/[RECALL] — session only
...
// Show the external scripts sub-toggle only when preview is enabled
function syncExtscriptsRow() {
  if (!els.toolExtscriptsRow) return;
  const previewOn = getToolEnabled('preview');
  els.toolExtscriptsRow.style.opacity    = previewOn ? '1' : '0.4';
  els.toolExtscriptsRow.style.pointerEvents = previewOn ? '' : 'none';
}
...
async function initProjects() {
  ...
  // First boot after Phase 11 — create Default project and migrate orphaned conversations
  if (!projects.length) {
    const def = {
      id:           generateId(),
      name:         'Default',
...
async function switchProject(projectId) {
  // Save lastConvId on outgoing project
  ...
  // No saved conversation — fresh chat
  currentConvId = null;
  currentMsgs   = [];
...
function renderProjectCards() {
  ...
  if (!projects.length) {
    els.projectCardsList.innerHTML = '<p class="ctx-empty">No projects yet.</p>';
    return;
  }
  ...
  for (const proj of projects) {
    ...
    card.innerHTML = `
      ...
      <div class="project-card-actions">
        ...
        ${projects.length > 1
          ? `<button class="key-card-btn key-card-del proj-del-btn" data-id="${proj.id}" title="Delete">✕</button>`
          : ''}
      </div>`;
...
async function exportProject(projId) {
  ...
  a.href      = url;
  a.download  = `vian-project-${proj.name.replace(/\s+/g, '-').toLowerCase()}.json`;
...
async function importProject(file) {
  try {
    ...
    if (data.vian_export !== 'project' || !data.project) {
      showToast('Invalid project file.'); return;
    }
    if (projects.length >= MAX_PROJECTS) {
      showToast(`Project limit reached (${MAX_PROJECTS} max).`); return;
    }
...
function openProjectPicker() {
  els.projectPicker.classList.remove('hidden');
  els.projectSwitcherBtn.setAttribute('aria-expanded', 'true');
}
function closeProjectPicker() {
  els.projectPicker.classList.add('hidden');
  els.projectSwitcherBtn.setAttribute('aria-expanded', 'false');
}
function toggleProjectPicker() {
  els.projectPicker.classList.contains('hidden') ? openProjectPicker() : closeProjectPicker();
}
...
async function persistConv() {
  ...
  // Save lastConvId on current project
  const proj = getCurrentProject();
  if (proj) {
    proj.lastConvId = currentConvId;
    proj.updatedAt  = Date.now();
    await saveProject(proj);
  }
  await loadHistory();
...
async function newChat() {
  // Enforce 10-conversation limit per project
  const convs = await getConversationsByProject(currentProjectId);
  if (convs.length >= MAX_CONVS) {
    showToast(`This project has reached its ${MAX_CONVS} conversation limit.`);
    return;
  }
...
async function doStream() {
  ...
  try {
    for await (const chunk of streamMessage({
      modelId, messages: currentMsgs, system,
      onUsage: (u) => { usage = u; },
    })) {
      fullText += chunk;
...
function handleStoreRecall(text) {
  // Process STORE blocks — save to sessionStore, strip from display
  let result = text.replace(STORE_RE, (match, key, value) => {
    ...
    if (sessionStore.size >= MAX_STORE_SLOTS) {
      showToast(`AI storage full (${MAX_STORE_SLOTS} slots max).`);
      return '';
    }
    const bytes = new TextEncoder().encode(v).length;
    if (bytes > MAX_SLOT_BYTES) {
      showToast(`Storage slot "${k}" exceeds 50 KB limit — truncated.`);
      sessionStore.set(k, v.slice(0, MAX_SLOT_BYTES));
    } else {
      sessionStore.set(k, v);
    }
    return ''; // strip block from rendered output
  });

  // Process RECALL blocks — replace inline with stored values
...
async function handleLineFetchBlocks(fullText, bubble) {
  ...
  for (const m of matches) {
    ...
    if (!rawUrl) {
      result = result.replace(m[0], `[Line fetch skipped: cannot resolve "${filePath}" — set a repo URL in the project settings]`);
      continue;
    }

    bubble.innerHTML = mdRender(stripBlocks(result).replace(
      m[0], `<span class="fetch-status">⟳ Fetching ${esc(filePath)} lines ${start}–${end}…</span>`
    ));

    let fetched;
    try {
      fetched = await fetchLineRange(rawUrl, start, end);
      showToast(`Fetched ${filePath} ✓`, 'success', 2000);
    } catch (err) {
      result = result.replace(m[0], `[Line fetch failed: ${err.message}]`);
      showToast('Line fetch failed: ' + err.message);
      continue;
    }
    ...
    currentMsgs.push({
      role:    'user',
      content: `[SYSTEM: Line range ${start}–${end} from ${filePath}]\n\n${fetched}\n\n[END FETCH]\n\nPlease continue your response.`,
    });
    await doStream();
  }
...
async function handleIndexBlocks(fullText, bubble) {
  ...
  for (const m of matches) {
    const repoUrl  = m[1].trim();
    const resolved = resolveUrl(repoUrl);

    if (!resolved) {
      result = result.replace(m[0], `[Index skipped: unrecognised URL]`);
      continue;
    }
    ...
    if (!indexText) {
      bubble.innerHTML = mdRender(stripBlocks(result).replace(
        m[0], `<span class="fetch-status">⟳ Building index for ${esc(repoUrl)}…</span>`
      ));
      try {
        ...
        result = result.replace(m[0], `[Index failed: ${err.message}]`);
...
async function handleFetchBlocks(fullText, bubble) {
  ...
  for (const m of matches) {
    ...
    if (!resolved) {
      result = result.replace(m[0], `[Fetch skipped: unrecognised URL — ${esc(originalUrl)}]`);
      continue;
    }

    bubble.innerHTML = mdRender(stripBlocks(result).replace(
      m[0], `<span class="fetch-status">⟳ Fetching ${esc(originalUrl)} via ${resolved.via}…</span>`
    ));
...
function renderPreviewBlocks(text) {
  ...
  return text.replace(PREVIEW_RE, (match, html) => {
    ...
    // When external scripts are off, inject a strict CSP meta tag
    if (!allowExternal) {
      ...
      if (content.includes('<head>')) {
        ...
        content = content.replace(/<head([^>]*)>/, `<head$1>${csp}`);
      } else {
        content = csp + content;
      }
    }
    ...
    // Return a placeholder marker — actual iframe injected after mdRender
    // We use a data attribute on a div so it survives HTML serialisation
...
function applyCodeBlocks(container) {
  container.querySelectorAll('pre code').forEach(codeEl => {
    ...
    body.className = 'code-block-body folded'; body.style.maxHeight = '0px';
...
function applyPreviewWidgets(container) {
  container.querySelectorAll('.preview-placeholder').forEach(placeholder => {
    const htmlContent = placeholder.dataset.html
      .replace(/&amp;/g, '&')
      .replace(/&quot;/g, '"');
    ...
    iframeWrap.className = 'preview-iframe-wrap hidden';

    wrap.appendChild(hdr);
    wrap.appendChild(iframeWrap);
    placeholder.replaceWith(wrap);

    const showBtn   = hdr.querySelector('.preview-show-btn');
    const expandBtn = hdr.querySelector('.preview-expand-btn');
    let   rendered  = false;
    let   expanded  = false;
...
function langToExt(lang) {
  const map = {
    ...
    json:'json', bash:'sh', shell:'sh', java:'java', kotlin:'kt',
    swift:'swift', rust:'rs', go:'go', c:'c', cpp:'cpp', markdown:'md',
    yaml:'yml', toml:'toml', xml:'xml', sql:'sql', php:'php', ruby:'rb', dart:'dart',
  };
  return map[lang] || 'txt';
}
...
function exportChat() {
  ...
  const url  = URL.createObjectURL(blob);
  const a    = document.createElement('a');
  a.href = url; a.download = `vian-${Date.now()}.md`;
  document.body.appendChild(a); a.click();
  document.body.removeChild(a); URL.revokeObjectURL(url);
}
...
function bindEvents() {
  ...
  els.btnApi.addEventListener('click', () => {
    hideDisambig(); els.keyPasteInput.value = '';
    renderKeyCards(); openModal('modal-api'); closeSidebar();
  });
  ...
  // Project switcher
  els.projectSwitcherBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    toggleProjectPicker();
  });
  els.btnManageProjects.addEventListener('click', () => {
    closeProjectPicker();
    renderProjectCards();
  ...
  document.addEventListener('click', (e) => {
    if (
      !els.projectPicker.classList.contains('hidden') &&
      !els.projectPicker.contains(e.target) &&
      !els.projectSwitcherBtn.contains(e.target)
    ) closeProjectPicker();
  });
  ...
  els.keyDisambig.querySelectorAll('.key-disambig-opt').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const key = pendingKey; hideDisambig(); commitKey(btn.dataset.provider, key);
    });
  });
  els.keyCardsList.addEventListener('click', (e) => {
    ...
    const i = Number(idx); const arr = getKeys(provider);
  ...
  if (els.toolPreview) els.toolPreview.addEventListener('change', () => {
  ...
  // Projects modal
  els.projNewPrompt.addEventListener('input', () => {
    els.projPromptCount.textContent = els.projNewPrompt.value.length;
  });
  els.btnCreateProject.addEventListener('click', async () => {
    const name   = els.projNewName.value.trim();
    const prompt = els.projNewPrompt.value.trim();
    const repo   = els.projNewRepo.value.trim();
    if (!name) { showToast('Project name is required.'); return; }
    if (projects.length >= MAX_PROJECTS) { showToast(`Project limit reached (${MAX_PROJECTS} max).`); return; }
    const proj = {
      id: generateId(), name, systemPrompt: prompt, repoUrl: repo,
  ...
  els.projectCardsList.addEventListener('click', async (e) => {
    ...
    if (editBtn) {
      const proj = projects.find(p => p.id === editBtn.dataset.id);
      if (proj) showProjectEditForm(proj);
    } else if (exportBtn) {
      await exportProject(exportBtn.dataset.id);
    } else if (delBtn) {
  ...
  // Import project
  els.projImportFile.addEventListener('change', async (e) => {
